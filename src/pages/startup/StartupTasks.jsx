  ...
  return (
    <div className="p-6 lg:p-8 max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <Button onClick={() => { setEditTask(null); setShowForm(true); }} className="gap-2 bg-violet-600 hover:bg-violet-700">
      ...
      <div className="space-y-2">
        {filtered.map(task => (
          <TaskItem key={task.id} task={task} projectName={projectMap[task.project_id]}
            onEdit={(t) => { setEditTask(t); setShowForm(true); }}
            onDelete={(t) => deleteMutation.mutate(t.id)}
            onStatusChange={(task, status) => updateMutation.mutate({ id: task.id, data: { ...task, status } })}
          />
        ))}
        {filtered.length === 0 && (
