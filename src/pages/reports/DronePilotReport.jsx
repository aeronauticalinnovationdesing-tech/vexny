...
${ctx}`,
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      ...
      {/* Gráficas fila 1: Pilotos + Drones */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div ref={refPilotStatus} className="bg-card rounded-2xl border border-border p-5">
          <h3 className="font-semibold mb-4 flex items-center gap-2"><Users className="w-4 h-4 text-sky-500" />Estado de Pilotos</h3>
          {pilotStatusData.length > 0 ? (
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                <Pie data={pilotStatusData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} label={({ name, value }) => `${name}: ${value}`} labelLine>
                  {pilotStatusData.map((_, i) => <Cell key={i} fill={[COLORS[0], COLORS[3], COLORS[6]][i]} />)}
                </Pie>
                <Tooltip /><Legend />
              </PieChart>
            </ResponsiveContainer>
          ) : <div className="h-[220px] flex items-center justify-center text-muted-foreground text-sm">Sin pilotos registrados</div>}
        </div>

        <div ref={refDroneStatus} className="bg-card rounded-2xl border border-border p-5">
          <h3 className="font-semibold mb-4 flex items-center gap-2"><Activity className="w-4 h-4 text-green-500" />Estado de la Flota</h3>
          {droneStatusData.length > 0 ? (
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                <Pie data={droneStatusData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={50} outerRadius={85} label={({ name, value }) => `${name}: ${value}`}>
                  {droneStatusData.map((_, i) => <Cell key={i} fill={[COLORS[1], COLORS[2], COLORS[3]][i]} />)}
                </Pie>
                <Tooltip /><Legend />
              </PieChart>
            </ResponsiveContainer>
          ) : <div className="h-[220px] flex items-center justify-center text-muted-foreground text-sm">Sin drones registrados</div>}
        </div>
      </div>

      {/* Gráficas fila 2: Horas por dron + Área de vuelo */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div ref={refHoursBar} className="bg-card rounded-2xl border border-border p-5">
          <h3 className="font-semibold mb-4 flex items-center gap-2"><BarChart2 className="w-4 h-4 text-purple-500" />Horas de Vuelo por Dron</h3>
          {droneHoursData.length > 0 ? (
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={droneHoursData} layout="vertical" barSize={16}>
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(220,14%,90%)" />
                <XAxis type="number" tick={{ fontSize: 10 }} unit="h" />
                <YAxis type="category" dataKey="name" tick={{ fontSize: 10 }} width={80} />
                <Tooltip formatter={v => `${v}h`} />
                <Bar dataKey="horas" name="Horas" radius={[0, 4, 4, 0]}>
                  {droneHoursData.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ) : <div className="h-[220px] flex items-center justify-center text-muted-foreground text-sm">Sin datos de horas</div>}
        </div>

        <div ref={refFlightArea} className="bg-card rounded-2xl border border-border p-5">
          <h3 className="font-semibold mb-4 flex items-center gap-2"><Shield className="w-4 h-4 text-amber-500" />Tipo de Área de Vuelo (SMS)</h3>
          {flightAreaData.length > 0 ? (
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                <Pie data={flightAreaData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} label={({ name, value }) => `${name}: ${value}`}>
                  {flightAreaData.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
                </Pie>
                <Tooltip /><Legend />
              </PieChart>
            </ResponsiveContainer>
          ) : <div className="h-[220px] flex items-center justify-center text-muted-foreground text-sm">Sin reportes SMS</div>}
        </div>
      </div>

      {/* Gráfica: SMS por mes */}
      <div ref={refSmsMonth} className="bg-card rounded-2xl border border-border p-5">
        <h3 className="font-semibold mb-4 flex items-center gap-2"><FileText className="w-4 h-4 text-red-500" />Reportes SMS por Mes (últimos 6)</h3>
        {smsMonthData.length > 0 ? (
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={smsMonthData}>
              <CartesianGrid strokeDasharray="3 3" stroke="hsl(220,14%,90%)" />
              <XAxis dataKey="mes" tick={{ fontSize: 11 }} />
              <YAxis tick={{ fontSize: 11 }} allowDecimals={false} />
              <Tooltip />
              <Legend />
              <Bar dataKey="vuelos" name="Vuelos" fill="#0ea5e9" radius={[4, 4, 0, 0]} />
              <Bar dataKey="incidentes" name="Incidentes" fill="#ef4444" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        ) : <div className="h-[220px] flex items-center justify-center text-muted-foreground text-sm">Sin datos mensuales SMS</div>}
      </div>

      {/* Gráfica: Pólizas */}
      <div ref={refMaintenancePie} className="bg-card rounded-2xl border border-border p-5">
        <h3 className="font-semibold mb-4 flex items-center gap-2"><Wrench className="w-4 h-4 text-purple-500" />Pólizas por Tipo de Cobertura</h3>
        {policyTypeData.length > 0 ? (
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={policyTypeData} barSize={50}>
              <CartesianGrid strokeDasharray="3 3" stroke="hsl(220,14%,90%)" />
              <XAxis dataKey="name" tick={{ fontSize: 12 }} />
              <YAxis tick={{ fontSize: 12 }} allowDecimals={false} />
              <Tooltip />
              <Bar dataKey="value" name="Pólizas" radius={[6, 6, 0, 0]}>
                {policyTypeData.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        ) : <div className="h-[200px] flex items-center justify-center text-muted-foreground text-sm">Sin pólizas registradas</div>}
      </div>
